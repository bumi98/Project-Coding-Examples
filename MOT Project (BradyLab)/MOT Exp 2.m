%For the second experiment, color changes half the speed (1 degree per
%frame now) and velocity is changing randomly whenenver they bounce. Coin
%flips are added as well.
...
for j = 1:nBlock
    ...
    for i = 1:nTrial
        ...
        %Make sure target and non target colors are at least 30 degrees
        %apart.
        ...
        if trialstruct(2,i) == 3
            v1 = 0.045; %v for initial velocity
            v2 = 0.045;
            lowerv = 0.025;
            upperv = 0.065;
        ...
        for k = 1:round(dispTime/(flipSpd*monitorFlipInterval)) %around 3 secs
            ...
            %Color
            targetColor = targetColor+1; %Change the color by 1 degree each iteration (This if for EXPERIMENT 2)
            if targetColor >= 360
                targetColor = 1;
            end
            
            nontargetColor = nontargetColor+1;
            ...
            %Left Location
            x1 = radius*cos(angle_start1)+Circle1Center(1);
            data(j).leftpath(i).fortrial(k,1) = x1;
            y1 = radius*sin(angle_start1)+Circle1Center(2);
            data(j).leftpath(i).fortrial(k,2) = y1;
            %Right Location
            x2 = radius*cos(angle_start2)+Circle2Center(1);
            data(j).rightpath(i).fortrial(k,1) = x2;
            y2 = radius*sin(angle_start2)+Circle2Center(2);
            data(j).rightpath(i).fortrial(k,2) = y2;
            
            %NOW FOR EXPERIMENT 2, when it bounces speed will vary
            %randomly.
            if v1 ~= 0 && v2 ~= 0
              %Boundary for left
              if angle_start1 >= 4.7139
                v1 = (upperv-lowerv)*rand(1,1)+lowerv;
                v1 = (-1)*v1; %Flip it when it's too big and going up too high
              elseif angle_start1 <= 1.5708
                v1 = (upperv-lowerv)*rand(1,1)+lowerv; %This is making the v positive anyway!
              end
              
              %Boundary for right
              if angle_start2 >= 7.85398
                v2 = (upperv-lowerv)*rand(1,1)+lowerv;
                v2 = (-1)*v2;
              elseif angle_start2 <= 4.71239 
                v2 = (upperv-lowerv)*rand(1,1)+lowerv;
              end
    ...
    save(fullfile('Data', ['2yongsstupidstudy_' sid 'mat']), 'data');
end
save(fullfile('Data', ['2yongsstupidstudy_' sid 'mat']), 'data');
