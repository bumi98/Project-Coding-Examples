...
for k = 1:numfiles
    myfilename = sprintf('yongsstupidstudy_%dmat.mat', k);
...
%Get rid of fifth one because the subject didn't complete it.
exclude = [5];
mu_ss1_stay(exclude) = [];
mu_ss1_move(exclude) = [];
mu_ss2_stay(exclude) = [];
...
% 
% figure(1);
% average_mu = [mu_ss1_stay_mean, mu_ss1_move_mean, mu_ss2_stay_mean, mu_ss2_move_mean];
% barName = {'SS1S', 'SS1M', 'SS2S', 'SS2M'};
% bar(average_mu);
% set(gca, 'XtickLabel', barName)
% 
% figure(2);
% average_sd = [sd_ss1_stay_mean, sd_ss1_move_mean, sd_ss2_stay_mean, sd_ss2_move_mean];
% barName = {'SS1S', 'SS1M', 'SS2S', 'SS2M'};
% bar(average_sd);
% set(gca, 'XtickLabel', barName)